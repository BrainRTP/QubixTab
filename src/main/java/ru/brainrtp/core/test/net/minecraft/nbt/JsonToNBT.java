package ru.brainrtp.core.test.net.minecraft.nbt;
...
public class JsonToNBT {
    ...
    public static NBTTagCompound getTagFromJson(String jsonString) throws Exception {
    ...
    static int topTagsCount(String str) throws Exception
    {
        int i = 0;
        boolean flag = false;
        ...
        for (int j = 0; j < str.length(); ++j)
        {
            ...
            if (c0 == 34)
            {
                if (isCharEscaped(str, j))
                {
                    if (!flag)
                    {
                        throw new Exception("Illegal use of \\\": " + str);
                    }
                }
                else
                {
            ...
            else if (!flag)
            {
                if (c0 != 123 && c0 != 91)
                {
                    ...
                    {
                        throw new Exception("Unbalanced curly brackets {}: " + str);
                    }

                    if (c0 == 93 && (stack.isEmpty() || ((Character)stack.pop()).charValue() != 91))
                    {
                        throw new Exception("Unbalanced square brackets []: " + str);
                    }
                }
                else
                {
        ...
        if (flag)
        {
            throw new Exception("Unbalanced quotation: " + str);
        }
        else if (!stack.isEmpty())
        {
            throw new Exception("Unbalanced brackets: " + str);
        }
        else
        {
            if (i == 0 && !str.isEmpty())
    ...
    static Any joinStrToNBT(String... args) throws Exception
    {
        return nameValueToNBT(args[0], args[1]);
    }

    static Any nameValueToNBT(String key, String value) throws Exception
    {
        value = value.trim();
        ...
        if (value.startsWith("{"))
        {
            ...
            for (jsontonbt$compound = new Compound(key); value.length() > 0; value = value.substring(s1.length() + 1))
            {
                ...
                {
                    throw new Exception("Unexpected token \'" + c1 + "\' at: " + value.substring(s1.length()));
                }
            }
        ...
        else if (value.startsWith("[") && !INT_ARRAY_MATCHER.matcher(value).matches())
        {
            ...
            for (jsontonbt$list = new List(key); value.length() > 0; value = value.substring(s.length() + 1))
            {
                ...
                {
                    throw new Exception("Unexpected token \'" + c0 + "\' at: " + value.substring(s.length()));
                }
            }
    ...
    private static Any getTagFromNameValue(String str, boolean isArray) throws Exception
    {
    ...
    private static String nextNameValuePair(String str, boolean isCompound) throws Exception
    {
        ...
        if (isCompound)
        {
            if (i == -1)
            {
                throw new Exception("Unable to locate name/value separator for string: " + str);
            }

            if (j != -1 && j < i)
            {
                throw new Exception("Name error at: " + str);
            }
        }
        else if (i == -1 || i > j)
        {
    ...
    private static String locateValueAt(String str, int index) throws Exception
    {
        Stack<Character> stack = new Stack<Character>();
        int i = index + 1;
        boolean flag = false;
    ...
    private static String locateName(String str, boolean isArray) throws Exception
    {
        if (isArray)
        {
            str = str.trim();
    ...
    private static String locateValue(String str, boolean isArray) throws Exception
    {
        if (isArray)
        {
            str = str.trim();
    ...
    abstract static class Any
        {
             ...
             */
            public abstract NBTBase parse() throws Exception;
        }
    ...
    static class Compound extends Any
        {
             ...
             * @return an {@link NBTBase} which can be safely cast to the type represented by this class.
             */
            public NBTBase parse() throws Exception
            {
